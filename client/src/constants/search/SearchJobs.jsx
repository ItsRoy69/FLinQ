...
import {
  dummyJobsSearchRecommendationArray,
  dummyEventsSearchRecommendationArray,
} from "../../data/DummySearchRecommendation";
    ...
    <div className="fixed top-0 left-0 h-screen w-screen dark:bg-custom-dark dark:text-white flex flex-col z-20">
      ...
      <div className="flex flex-col gap-1">
        {searchRecommendation.map((item, index) => (
          <div
            className="h-14 w-full flex items-center px-3 rounded-2xl"
            key={index}
          >
            <p className="text-sm font-light">{item.search}</p>
          </div>
        ))}
      </div>
    </div>
